...
local function main()
	...
	BASE = require("Play.lua.base").createBasePositionDP(14)
	-- テキスト
	...
	skin.pmchara = {}
	skin.destination = {}
	require("Play.lua.base").addSourceDP(skin)
	-- 背景
	do
	...
	do
		...
		local keyflash_status, keyflash_parts = pcall(function()
			return dofile(keyflash_path).load(14)
		end)
	...
	do
		...
		local inputkey_status, inputkey_parts = pcall(function()
			return dofile(inputkey_path).load(14)
		end)
	...
	do
		...
		local notes_status, notes_parts = pcall(function()
			return dofile(notes_path).load(14)
		end)
	...
	-- スコアバー
	do
		local scorebar_path = skin_config.get_path("Play/lua/dp/scorebar.lua")
	...
	do
		...
		local bomb7_status, bomb7_parts = pcall(function()
			return dofile(bomb7_path).load(14)
		end)
	...
	table.insert(skin.destination, {
		id = MAIN.IMAGE.BLACK, timer = 2, loop = 500, dst = {
