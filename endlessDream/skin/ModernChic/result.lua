...
local function main()
	...
	skin.customEvents = {}
	skin.destination = {}
	if CONFIG.infoOutput then CUSTOM.FUNC.infoOutput(7) end
	...
	-- インプレ機能
	do
		local path = skin_config.get_path("Result/lua/impression.lua")
		local status, parts = pcall(function()
			return dofile(path).load()
		end)
		if status and parts then
	...
	-- 自作難易度表への追加機能
	-- TODO 楽曲情報をJSON形式で書き出す
	do
		local path = skin_config.get_path("Result/lua/diflist.lua")
		local status, parts = pcall(function()
			return dofile(path).load()
		end)
		if status and parts then
	...
	if CONFIG.voice.result.sw then
		CUSTOM.SOUND.resultVoice()
	end
	return skin
end
