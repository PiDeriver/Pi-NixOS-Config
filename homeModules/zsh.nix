{ config, ... }: 
{
  programs.zsh = {
    ...
    shellAliases = {
      ...
      updateRollback = "(cd $HOME/.dotfiles/; sudo nixos-rebuild switch --rollback --impure --flake . --option cores 8 --option max-jobs 6 2>&1 | grep -E -v 'Added input|follows|github:')";
