...
local function load(imageId)
	...
	table.insert(parts.destination,	{
		id = "close_msg_window", loop = 800, timer = MAIN.TIMER.FAILED, dst = {
			{time = 500, x = 0, y = 540, w = 1920, h = 0, acc = MAIN.ACC.ACCELERATION},
	...
	table.insert(parts.destination,	{
		id = "close_word_stage", loop = 3000, timer = MAIN.TIMER.FAILED, dst = {
			...
			{time = 1500, x = 280, a = 255, acc = MAIN.ACC.CONSTANT},
			{time = 3000, x = 250},
		}
	})
	table.insert(parts.destination,	{
		id = "close_word_failed", loop = 3000, timer = MAIN.TIMER.FAILED, dst = {
			...
			{time = 1500, x = 940, a = 255, acc = MAIN.ACC.CONSTANT},
			{time = 3000, x = 970},
		}
	})
	...
	if main_state.option(MAIN.OP.AUTOPLAYOFF) then
		if main_state.option(-MAIN.OP.MODE_COURSE) then
			table.insert(parts.destination, {
		...
		for i = 1, num, 1 do
			...
			table.insert(parts.destination,{
				id = "close_beam", timer = MAIN.TIMER.FAILED, loop = -1, dst = {
					{time = 700 + delayTime, x = -19, y = 688, w = 19, h = 19, acc = MAIN.ACC.ACCELERATION},
					{time = 1500 + delayTime, x = 1939}
				}
			})
			table.insert(parts.destination,{
				id = "close_beam", timer = MAIN.TIMER.FAILED, loop = -1, dst = {
					{time = 700 + delayTime, x = 1939, y = 372, w = 19, h = 19, acc = MAIN.ACC.ACCELERATION},
	...
	-- フェードアウト処理
	table.insert(parts.destination,	{
		id = MAIN.IMAGE.BLACK, timer = MAIN.TIMER.FAILED, loop = 3000, dst = {
