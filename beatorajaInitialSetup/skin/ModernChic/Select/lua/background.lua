...
local function load()
    ...
    if PROPERTY.isBgImage() then
		if PROPERTY.isbgRotationOff() then
			table.insert(parts.source, {id = 1, path = "Select/bg/image/*.png"})
		elseif PROPERTY.isbgRotationOn() then
			table.insert(parts.source, {id = 1, path = CUSTOM.FUNC.randomChoice("Select/bg/image/", "io/Select/bg/image/", "Select.+%.png")})
		end
    ...
    elseif PROPERTY.isBgMovie() then
		...
		elseif PROPERTY.isbgRotationOn() then
			table.insert(parts.source, {id = 1, path = CUSTOM.FUNC.randomChoice("Select/bg/movie/", "io/Select/bg/movie/", "Select.+%.mp4")})
		end
        table.insert(parts.image, {id = "background", src = 1, x = 0, y = 0, w = 1920, h = 1080})
    end
