...
local function main()
	...
	BASE = require("Play.lua.base").createBasePositionDP(10)
	-- テキスト
	...
	do
		...
		local keyflash_status, keyflash_parts = pcall(function()
			return dofile(keyflash_path).load(10)
		end)
	...
	do
		...
		local inputkey_status, inputkey_parts = pcall(function()
			return dofile(inputkey_path).load(10)
		end)
	...
	do
		...
		local notes_status, notes_parts = pcall(function()
			return dofile(notes_path).load(10)
		end)
	...
	if PROPERTY.is10keyLanecoverOn() then
		...
		table.insert(skin.destination,{
			id = "10keysFrame", loop = 1500, dst = {
				...
				{time = 1000},
				{time = 1500, y = BASE.NOTES_JUDGE_Y - 47}
			}
		})
		table.insert(skin.destination,{
			id = "10keysFrame", loop = 1500, dst = {
	...
	do
		...
		local bomb7_status, bomb7_parts = pcall(function()
			return dofile(bomb7_path).load(10)
		end)
