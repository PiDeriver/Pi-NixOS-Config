...
local function main()
	...
	BASE = require("Play.lua.base").createBasePositionSP(7)
	-- テキスト
	...
	do
		...
		local keyflash_status, keyflash_parts = pcall(function()
			return dofile(keyflash_path).load(7)
		end)
	...
	do
		...
		local inputkey_status, inputkey_parts = pcall(function()
			return dofile(inputkey_path).load(7)
		end)
	...
	do
		...
		local notes_status, notes_parts = pcall(function()
			return dofile(notes_path).load(7)
		end)
	...
	do
		...
		local bomb7_status, bomb7_parts = pcall(function()
			return dofile(bomb7_path).load(7)
		end)
	...
	if PROPERTY.isDetailInfoSwitchOn() then
		...
		local bgaareainfo_parts = dofile(bgaareainfo_path).load(8)
